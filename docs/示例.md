追番日程 Flutter App 开发文档（Flutter + 自托管 Supabase + MCP + DeepSeek）
...
16. 你可以直接交给 AI 开发的“执行级提示词”（可选，便于你分模块让 AI 写代码）

下面给你一个可复制的提示词骨架，你每次把“模块名 + 约束”替换即可：

提示词模板：
	•	目标：实现 Flutter 功能模块 XXX
	•	技术栈：Flutter 3.x, Riverpod, go_router, Dio, cached_network_image, flutter_animate
	•	UI 风格：米白自然色 + 艺术标题字体 + 流畅动画（页面切换、卡片展开、skeleton）
	•	代码要求：
	•	必须可编译运行
	•	分层：data/domain/ui
	•	DTO 与 API Client 必须完整
	•	错误处理与空态/loading 必须完整
	•	接口：
	•	列出本模块用到的 REST/SSE 接口
	•	输出：
	•	给出文件结构 + 完整代码（按文件分别输出）
